...
/**
 * GiyaAuth login driver for Giya User360 (Registration-as-a-service) platform
 *
 * @package     Fuel
 * @subpackage  Auth
 */
class Auth_Login_Gigyaauth extends \Auth_Login_Driver
{
	/*
	...
	public static function _init()
	{
		...
		\Config::load('gigyaauth', true, true, true);
		
		// test to see if the Gigya PHP SDK is loaded
		if(!class_exists('Gigya\\GSRequest')) {
			throw new \AuthException('Gigya PHP SDK required for GiyaAuth. Please download from: http://developers.gigya.com/030_Server_SDKs/PHP or via Composer.');
		}
	}

	/**
	 * @var  \Model\Gigya_User  user object for the current user
	 */
	protected $user = null;
	...
	protected $config = array(
		'drivers' => array('group' => array('Gigyagroup')),
	...
	public function validate_user($username_or_email = '', $password = '')
	{
		...
		$username_or_email = trim($username_or_email) ?: trim(\Input::post(\Config::get('gigya.username_post_key', 'username')));
		$password = trim($password) ?: trim(\Input::post(\Config::get('gigya.password_post_key', 'password')));
		...
		$user = new \Model\Auth_GigyaUser($gigya_account);

		// return the user object, or false if not found
		return !empty($user->id) ? $user : null; // TO DO: See if I can get Auth_GigyaUser returning null if empty
	}
	...
	public function create_user($username, $password, $email, $group = 1, Array $profile_fields = array())
	{
		...
		// Initialize Gigya Account API
		$gigya_account = new \Gigya\Model\GigyaAccount();
		
		// check if we already have an account with this email address or username
		// http://developers.gigya.com/037_API_reference/020_Accounts/accounts.isAvailableLoginID
		//$duplicate_email_response = $gigya_account->isAvailableLoginID($email);
		//$duplicate_username_response = $gigya_account->isAvailableLoginID($username);

		// did we find one?
		/*
		if(!(bool)$duplicate_email_response->getInt('isAvailable'))
		{
		...
		else if(!(bool)$duplicate_username_response->getInt('isAvailable')) 
		{
			throw new \SimpleUserUpdateException(
		...
		*/
		...
		// initialize registration, acquire regToken
		$gigya_account->initRegistration();
				
		if(!empty($gigya_account->regToken)) {
			// http://developers.gigya.com/037_API_reference/020_Accounts/accounts.register
			$gigya_account->register(
				(string)$username,
				$email,
				$this->hash_password((string)$password),
				$gigya_account->regToken,
				array(
					'firstName' => $profile_fields['first_name'],
					'lastName' => $profile_fields['last_name'],
					'nickname' => $username,
					'email' => $email,
	...
	protected function perform_check()
	{
		...
		if ( ! empty($username) and ! empty($login_hash))
		{
			...
			if ($this->user and (\Config::get('gigyaauth.multiple_logins', false) or $this->user['login_hash'] === $login_hash))
			{
				return true;
			}
		}
